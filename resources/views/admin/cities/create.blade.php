...
@section('title', 'Yangi Shahar')
@section('page-title', 'Yangi Shahar Qo\'shish')
    ...
    <form action="{{ route('admin.cities.store') }}" method="POST">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="region" class="form-control @error('region') is-invalid @enderror" value="{{ old('region') }}" placeholder="Masalan: Toshkent viloyati">
                @error('region')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6">
                ...
                <input type="number" step="0.00000001" name="latitude" class="form-control @error('latitude') is-invalid @enderror" value="{{ old('latitude') }}" placeholder="41.311081">
                @error('latitude')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6">
                ...
                <input type="number" step="0.00000001" name="longitude" class="form-control @error('longitude') is-invalid @enderror" value="{{ old('longitude') }}" placeholder="69.240562">
                @error('longitude')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
