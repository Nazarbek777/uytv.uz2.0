    ...
    <div>
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-3 col-md-4">
                    <div class="footer-widget">
                        <a class="nav-footer-logo" href="{{ route('home') }}">
										...
										</span><h5 class="fs-2 fw-semibold ms-1 my-0">UyTv</h5>
                        </a>
                        <div class="footer-add">
                            @php
                                $locale = app()->getLocale();
                            @endphp
                            <p>{{ $locale === 'uz' ? 'O\'zbekiston, Toshkent shahri' : ($locale === 'ru' ? 'Узбекистан, город Ташкент' : 'Uzbekistan, Tashkent city') }}</p>
                ...
                <div class="col-lg-2 col-md-4">
                    <div class="footer-widget">
                        @php
                            $locale = app()->getLocale();
                        @endphp
                        <h4 class="widget-title">{{ $locale === 'uz' ? 'Navigatsiya' : ($locale === 'ru' ? 'Навигация' : 'Navigation') }}</h4>
                        <ul class="footer-menu">
                            <li><a href="/">{{ $locale === 'uz' ? 'Bosh sahifa' : ($locale === 'ru' ? 'Главная' : 'Home') }}</a></li>
                            <li><a href="{{ route('listings') }}">{{ $locale === 'uz' ? 'Uy-joylar' : ($locale === 'ru' ? 'Недвижимость' : 'Listings') }}</a></li>
                            <li><a href="{{ route('map') }}">{{ $locale === 'uz' ? 'Xarita' : ($locale === 'ru' ? 'Карта' : 'Map') }}</a></li>
                            <li><a href="#">{{ $locale === 'uz' ? 'Biz haqimizda' : ($locale === 'ru' ? 'О нас' : 'About Us') }}</a></li>
                            <li><a href="#">{{ $locale === 'uz' ? 'Aloqa' : ($locale === 'ru' ? 'Контакты' : 'Contact') }}</a></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="col-lg-2 col-md-4">
                    <div class="footer-widget">
                        <h4 class="widget-title">{{ $locale === 'uz' ? 'Uy turlari' : ($locale === 'ru' ? 'Типы недвижимости' : 'Property Types') }}</h4>
                        <ul class="footer-menu">
                            <li><a href="{{ route('listings') }}?property_type=apartment">{{ $locale === 'uz' ? 'Kvartira' : ($locale === 'ru' ? 'Квартира' : 'Apartment') }}</a></li>
                            <li><a href="{{ route('listings') }}?property_type=house">{{ $locale === 'uz' ? 'Uy' : ($locale === 'ru' ? 'Дом' : 'House') }}</a></li>
                            <li><a href="{{ route('listings') }}?property_type=villa">{{ $locale === 'uz' ? 'Villa' : ($locale === 'ru' ? 'Вилла' : 'Villa') }}</a></li>
                            <li><a href="{{ route('listings') }}?property_type=land">{{ $locale === 'uz' ? 'Yer' : ($locale === 'ru' ? 'Земля' : 'Land') }}</a></li>
                            <li><a href="{{ route('listings') }}?property_type=commercial">{{ $locale === 'uz' ? 'Savdo' : ($locale === 'ru' ? 'Коммерческая' : 'Commercial') }}</a></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="col-lg-2 col-md-6">
                    <div class="footer-widget">
                        <h4 class="widget-title">{{ $locale === 'uz' ? 'Mening hisobim' : ($locale === 'ru' ? 'Мой аккаунт' : 'My Account') }}</h4>
                        <ul class="footer-menu">
                            @auth
                            <li><a href="{{ route('provider.properties.index') }}">{{ $locale === 'uz' ? 'Dashboard' : ($locale === 'ru' ? 'Панель управления' : 'Dashboard') }}</a></li>
                            <li><a href="{{ route('provider.properties.index') }}">{{ $locale === 'uz' ? 'Mening uy-joylarim' : ($locale === 'ru' ? 'Моя недвижимость' : 'My Properties') }}</a></li>
                            <li><a href="{{ route('properties.create') }}">{{ $locale === 'uz' ? 'Uy-joy qo\'shish' : ($locale === 'ru' ? 'Добавить недвижимость' : 'Add Property') }}</a></li>
                            @else
                            <li><a href="#" data-bs-toggle="modal" data-bs-target="#login">{{ $locale === 'uz' ? 'Kirish' : ($locale === 'ru' ? 'Войти' : 'Login') }}</a></li>
                            <li><a href="{{ route('properties.create') }}">{{ $locale === 'uz' ? 'Uy-joy qo\'shish' : ($locale === 'ru' ? 'Добавить недвижимость' : 'Add Property') }}</a></li>
                            @endauth
                        </ul>
                    </div>
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="footer-widget">
                        <h4 class="widget-title">{{ $locale === 'uz' ? 'Ilovalarni yuklab olish' : ($locale === 'ru' ? 'Скачать приложения' : 'Download Apps') }}</h4>
                        <a href="#" class="other-store-link">
                            <div class="other-store-app">
                                ...
                                <div class="os-app-caps">
                                    {{ $locale === 'uz' ? 'Google Play' : ($locale === 'ru' ? 'Google Play' : 'Google Play') }}
                                    <span>{{ $locale === 'uz' ? 'Yuklab olish' : ($locale === 'ru' ? 'Скачать' : 'Get It Now') }}</span>
                                </div>
                            </div>
                        </a>
                        <a href="#" class="other-store-link">
                            <div class="other-store-app">
                                ...
                                <div class="os-app-caps">
                                    {{ $locale === 'uz' ? 'App Store' : ($locale === 'ru' ? 'App Store' : 'App Store') }}
                                    <span>{{ $locale === 'uz' ? 'Mavjud' : ($locale === 'ru' ? 'Доступно' : 'Now Available') }}</span>
                                </div>
                            </div>
                        </a>
                    </div>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-6 col-md-6">
                    @php
                        $locale = app()->getLocale();
                    @endphp
                    <p class="mb-0">© {{ date('Y') }} UyTv. {{ $locale === 'uz' ? 'Barcha huquqlar himoyalangan' : ($locale === 'ru' ? 'Все права защищены' : 'All Rights Reserved') }}</p>
                </div>
