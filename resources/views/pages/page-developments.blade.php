                                ...
                                <div class="mb-2">
                                    @php
                                        ...
                                        if ($development->properties && $development->properties->count() > 0) {
                                            foreach ($development->properties->groupBy('bedrooms') as $roomCount => $props) {
                                                $propertiesByRoom[$roomCount] = [
                                    ...
                                    @endphp
                                    @foreach($propertiesByRoom as $roomCount => $details)
                                    <div class="d-flex justify-content-between align-items-center mb-1" style="font-size: 12px;">
                                        <span class="text-muted">{{ $roomCount }} {{ $locale === 'uz' ? 'xona' : ($locale === 'ru' ? 'комн.' : 'room') }}. {{ $locale === 'uz' ? 'dan' : ($locale === 'ru' ? 'от' : 'from') }} {{ number_format($details['area_from'], 2) }} m²</span>
