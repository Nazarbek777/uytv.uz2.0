...
Route::middleware(['auth'])->prefix('provider')->name('provider.')->group(function () {
    ...
    Route::post('/onboarding/company', [OnboardingController::class, 'updateCompanyInfo'])->name('onboarding.company');
    Route::post('/onboarding/documents', [OnboardingController::class, 'uploadDocuments'])->name('onboarding.documents');
    Route::post('/onboarding/submit', [OnboardingController::class, 'submitForReview'])->name('onboarding.submit');

    Route::middleware('provider.onboarded')->group(function () {
        Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');
        Route::get('/', fn () => redirect()->route('provider.dashboard'))->name('home');
        Route::get('/settings', [SettingsController::class, 'index'])->name('settings.index');
        Route::post('/settings/profile', [SettingsController::class, 'updateProfile'])->name('settings.profile');
        Route::post('/settings/notifications', [SettingsController::class, 'updateNotifications'])->name('settings.notifications');
        Route::post('/settings/password', [SettingsController::class, 'updatePassword'])->name('settings.password');

        Route::get('/subscriptions', [\App\Http\Controllers\Provider\SubscriptionController::class, 'index'])->name('subscriptions.index');
        Route::post('/subscriptions/{plan}/subscribe', [\App\Http\Controllers\Provider\SubscriptionController::class, 'subscribe'])->name('subscriptions.subscribe');
        Route::post('/subscriptions/boost', [\App\Http\Controllers\Provider\SubscriptionController::class, 'boost'])->name('subscriptions.boost');
        ...
        Route::post('/properties/{id}/submit', [PropertyController::class, 'submitForApproval'])->name('properties.submit');
        Route::post('/properties/{id}/withdraw', [PropertyController::class, 'withdrawSubmission'])->name('properties.withdraw');
    });
});
...
Route::middleware(['auth', \App\Http\Middleware\AdminMiddleware::class])->prefix('admin')->name('admin.')->group(function () {
    ...
    // AI Analytics
    Route::get('/ai/chatbot-analytics', [\App\Http\Controllers\Admin\AiChatbotAnalyticsController::class, 'index'])->name('ai.chatbot-analytics.index');
    Route::get('/ai/chatbot-analytics/{aiChatbotLog}', [\App\Http\Controllers\Admin\AiChatbotAnalyticsController::class, 'show'])->name('ai.chatbot-analytics.show');
    Route::get('/ai/search-analytics', [\App\Http\Controllers\Admin\AiSearchAnalyticsController::class, 'index'])->name('ai.search-analytics.index');
    Route::get('/ai/search-analytics/{aiSearchLog}', [\App\Http\Controllers\Admin\AiSearchAnalyticsController::class, 'show'])->name('ai.search-analytics.show');

    // AI Content Generator
    Route::get('/ai/content-generator', [\App\Http\Controllers\Admin\AiContentGeneratorController::class, 'index'])->name('ai.content-generator.index');
    Route::post('/ai/content-generator/generate', [\App\Http\Controllers\Admin\AiContentGeneratorController::class, 'generate'])->name('ai.content-generator.generate');

    // AI Fraud Detection
    Route::get('/ai/fraud-detection', [\App\Http\Controllers\Admin\AiFraudDetectionController::class, 'index'])->name('ai.fraud-detection.index');
    Route::post('/ai/fraud-detection/scan', [\App\Http\Controllers\Admin\AiFraudDetectionController::class, 'scan'])->name('ai.fraud-detection.scan');
    Route::post('/ai/fraud-detection/{aiFraudDetection}/review', [\App\Http\Controllers\Admin\AiFraudDetectionController::class, 'review'])->name('ai.fraud-detection.review');

    // AI Price Predictor
    Route::get('/ai/price-predictor', [\App\Http\Controllers\Admin\AiPricePredictorController::class, 'index'])->name('ai.price-predictor.index');
    Route::post('/ai/price-predictor/predict', [\App\Http\Controllers\Admin\AiPricePredictorController::class, 'predict'])->name('ai.price-predictor.predict');
