...
class AiPropertySearchService
{
    ...
    protected function extractFiltersFromQueryFallback(string $query, string $locale): array
    {
        ...
        // Bedrooms extract (masalan: "2 xonali", "3 xona", "2 xonalik", "2 bedrooms")
        // Pattern 1: "2 xonali", "2 xonalik", "2 xona"
        if (preg_match('/(\d+)\s*(?:xonali|xonalik|xona|комнат|bedroom|bed|room)/i', $query, $matches)) {
            $bedrooms = (int)$matches[1];
            if ($bedrooms > 0 && $bedrooms <= 10) {
                $filters['bedrooms'] = $bedrooms;
            }
        } 
        // Pattern 2: "xonali 2", "xona 3"
        elseif (preg_match('/(?:xonali|xonalik|xona|комнат|bedroom|bed|room).*?(\d+)/i', $query, $matches)) {
            $bedrooms = (int)$matches[1];
            if ($bedrooms > 0 && $bedrooms <= 10) {
                $filters['bedrooms'] = $bedrooms;
            }
        } 
        // Pattern 3: "2-xonali", "3-xona"
        elseif (preg_match('/(\d+)[\s-]*(?:xonali|xonalik|xona)/i', $query, $matches)) {
            $bedrooms = (int)$matches[1];
            if ($bedrooms > 0 && $bedrooms <= 10) {
                $filters['bedrooms'] = $bedrooms;
            }
        }
        // Pattern 4: "ikki xonali", "uch xona" (o'zbek raqamlari)
        elseif (preg_match('/(?:bir|ikki|uch|to\'rt|besh|olti|yetti|sakkiz|to\'qqiz|o\'n)\s*(?:xonali|xonalik|xona)/i', $query, $matches)) {
            $numberMap = [
                'bir' => 1, 'ikki' => 2, 'uch' => 3, 'to\'rt' => 4, 'besh' => 5,
                'olti' => 6, 'yetti' => 7, 'sakkiz' => 8, 'to\'qqiz' => 9, 'o\'n' => 10
            ];
            $numberWord = mb_strtolower($matches[1]);
            if (isset($numberMap[$numberWord])) {
                $filters['bedrooms'] = $numberMap[$numberWord];
            }
        }
