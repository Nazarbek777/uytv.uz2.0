...
class PropertyController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $property->appendApprovalHistory('draft', [
            'action' => 'created',
            'actor' => Auth::user()->name ?? 'Provider',
        ]);
        $property->save();
        ...
        if ($subscription = $this->subscriptionService->getActiveSubscription(Auth::user())) {
            $this->subscriptionService->consumeListingSlot($subscription);
        }
    ...
    public function submitForApproval(string $id)
    {
        ...
        if (! in_array($property->status, ['draft', 'rejected'])) {
            return back()->with('error', __('Faqat qoralama yoki rad etilgan e\'lonlarni yuborish mumkin.'));
        }

        $property->status = 'pending';
        $property->approval_status = 'pending';
        $property->approval_submitted_at = now();
        $property->approval_reviewed_at = null;
        $property->approval_reviewer_id = null;
        $property->approval_notes = null;
        $property->appendApprovalHistory('pending', [
            'action' => 'submitted',
            'actor' => Auth::user()->name ?? 'Provider',
        ]);
        $property->save();

        return back()->with('success', __('E\'lon tasdiqlashga yuborildi.'));
    }

    /**
     * Withdraw a pending submission back to draft.
     */
    public function withdrawSubmission(string $id)
    {
        $property = Property::where('id', $id)
            ->where('user_id', Auth::id())
            ->firstOrFail();
        ...
        if ($property->status !== 'pending') {
            return back()->with('error', __('Faqat tasdiqlash jarayonidagi e\'lonni bekor qilish mumkin.'));
        }
        ...
        return back()->with('success', __('Tasdiqqa yuborish bekor qilindi.'));
    }
